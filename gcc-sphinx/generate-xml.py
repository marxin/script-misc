...
subprocess.check_output(f'{cmd} {includes} {SRCDIR}/gcc/fortran/gfortran.texi -I{SRCDIR}/gcc/fortran -o {OUTPUT}/gfortran.xml', shell=True)
subprocess.check_output(f'{cmd} {includes} {SRCDIR}/gcc/go/gccgo.texi -I{SRCDIR}/gcc/go -o {OUTPUT}/gccgo.xml', shell=True)
...
for lib in ('libgomp', 'libquadmath', 'libitm'):
    subprocess.check_output(f'{cmd} {includes} -I{OBJDIR}/x86_64-pc-linux-gnu/libquadmath/ {SRCDIR}/{lib}/{lib}.texi -o {OUTPUT}/{lib}.xml', shell=True)
